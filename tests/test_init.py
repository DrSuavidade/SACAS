...
def test_init_creates_default_structure_root_and_canonical_manifest(tmp_path: Path) -> None:
    ...
    assert "sacas prepare" in (tmp_path / "Structure" / "ROUTER.md").read_text(encoding="utf-8")
...
def test_init_refuses_repository_root_placement_before_writing(tmp_path: Path) -> None:
    from sacas.init import initialize
...
def test_reinit_preserves_human_content(tmp_path: Path) -> None:
    from sacas.init import initialize

    initialize(tmp_path)
    human_context = tmp_path / "Structure" / "references" / "notes.md"
    human_context.write_text("# Human workflow note\n", encoding="utf-8")

    initialize(tmp_path)

    assert human_context.read_text(encoding="utf-8") == "# Human workflow note\n"
